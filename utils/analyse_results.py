...
from utils.plot_setup import set_style, pltmarkers as MARKERS, fontsizelabels as FSIZELABELS, fontsizeticks as FSIZETICKS
...
def load_results_linkage(dirname):
    ...
    files = glob(path.join(dirname, f'VAR_result_try3_new.json'))
...
def load_results_inference(dirname, dpath):
    ...
    files = glob(path.join(dirname, f'VAR_inference_result_try3_new.json'))
    resList = []
    for fpath in files:
...
def load_results_utility(dirname):
    ...
    files = glob(path.join(dirname, f'utilityresult.json'))
