...
def main():
    ...
    ######### EVALUATION #############
    ##################################
    resultsTargetPrivacy = {tid: {sa: {gm.__name__: {} for gm in gmList} for sa in runconfig['sensitiveAttributes']} for tid in targetIDs}
    # Add entry for raw
    ...
    LOGGER.info(f"Write results to {path.join(f'{args.outdir}', f'VAR_inference_result_try3_new')}")

    with open(path.join(f'{args.outdir}', f'VAR_inference_result_try3_new.json'), 'w') as f:
