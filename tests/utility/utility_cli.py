...
def main():
    ...
    argparser.add_argument('--outdir', '-O', default='outputs', type=str, help='Path relative to cwd for storing output files')
    ...
    ######### EVALUATION #############
    ##################################
    resultsTargetUtility = {ut.__name__: {gm.__name__: {} for gm in gmList } for ut in utilityTasks}
    ...
    with open(path.join(f'{args.outdir}', f'VAR_result.json'), 'w') as f:
